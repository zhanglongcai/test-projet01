...
export function WeChatMPLogin() {
  const [isLoading, setIsLoading] = useState(false);

  const handleMPLogin = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/auth/wechat/mp/auth-url?' + new URLSearchParams({
        redirectUri: `${window.location.origin}/api/auth/wechat/mp/callback`,
